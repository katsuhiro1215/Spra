...
1. ✅ ガントチャート内でのドラッグ＆ドロップ編集（日付変更・進捗変更、サーバー保存済み）
2. ✅ ProjectItemのドラッグ＆ドロップ並び替え（`items/reorder`エンドポイントで`sort_order`を保存）
3. ✅ ファイルアップロード機能（`ProjectFile`モデル、`private`ディスク保存、ダウンロード・削除対応）
4. ✅ ProjectUpdate作成フォーム（Project詳細画面「更新履歴」タブにモーダルフォームを追加）

## 🚧 今後の実装予定

1. ❌ マイルストーンのドラッグ＆ドロップ並び替え（ガントチャート上ではマイルストーンはタイムラインの旗マーカー表示のみで、並び替えUI自体が未実装）
