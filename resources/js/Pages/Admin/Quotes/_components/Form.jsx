...
export default function QuoteForm({
    ...
    /**
     * ServicePlanを選択したときに、関連するServiceItemsを一括追加
     */
    const handleAddServicePlan = (servicePlan) => {
        console.log("handleAddServicePlan called with:", servicePlan);
        console.log("servicePlan.service_items:", servicePlan.service_items);
        ...
        // ServicePlan内の全Itemsを明細に追加（表示用）
        // ただしplan情報（base_price）を持たせる
        const newItems = servicePlan.service_items.map((serviceItem, index) => {
            ...
            return {
                ...
                service_plan_base_price: servicePlan.base_price,
                service_item_id: serviceItem.id,
                name: serviceItem.name,
                description: serviceItem.description || "",
                item_type: serviceItem.item_type,
        ...
        // 追加後の合計金額を計算
        const currentBaseAmount = items.reduce(
            (sum, item) => sum + (parseFloat(item.amount) || 0),
            0,
        );
        const planItemsAmount = newItems.reduce(
            (sum, item) => sum + (parseFloat(item.amount) || 0),
            0,
        );
        const totalAfterAdd = currentBaseAmount + planItemsAmount;

        // 値引き額を計算（プラン基本価格がこれ以上にならないように）
        const planBasePrice = parseFloat(servicePlan.base_price);
        let discountAmount = 0;
        if (totalAfterAdd > planBasePrice) {
            discountAmount = totalAfterAdd - planBasePrice;
        }

        console.log("Adding items to quote:", newItems);
    ...
    const getFilteredServicePlans = () => {
        ...
        if (selectedPlanCategory) {
            // servicePlanはservice経由でcategoryに繋がっているはずだが、ここではserviceに絞る
            // service_idで絞り込む
