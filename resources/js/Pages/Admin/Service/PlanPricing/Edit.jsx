...
export default function PlanPricingEdit({ servicePlan, planPricing }) {
    const { data, setData, patch, processing, errors } = useForm({
        name: planPricing.name || "",
        type: planPricing.type || "base",
        description: planPricing.description || "",
        price: planPricing.price || "",
        price_unit: planPricing.price_unit || "",
        billing_type: planPricing.billing_type || "fixed",
        min_quantity: planPricing.min_quantity || "",
        max_quantity: planPricing.max_quantity || "",
        tier_rules: planPricing.tier_rules || [],
        tier_start_quantity: planPricing.tier_start_quantity || "",
        tier_end_quantity: planPricing.tier_end_quantity || "",
        discount_percentage: planPricing.discount_percentage || "",
        discount_amount: planPricing.discount_amount || "",
        discount_start_date: planPricing.discount_start_date || "",
        discount_end_date: planPricing.discount_end_date || "",
        conditions: planPricing.conditions || [],
        requires_approval: planPricing.requires_approval || false,
        approval_notes: planPricing.approval_notes || "",
        is_default: planPricing.is_default || false,
        is_negotiable: planPricing.is_negotiable || false,
        is_recurring: planPricing.is_recurring || false,
        recurring_months: planPricing.recurring_months || "",
        is_visible: planPricing.is_visible ?? true,
        is_featured: planPricing.is_featured || false,
        sort_order: planPricing.sort_order || 0,
        display_name: planPricing.display_name || "",
        is_active: planPricing.is_active ?? true,
        effective_from: planPricing.effective_from
            ? planPricing.effective_from.slice(0, 16)
            : "",
        effective_until: planPricing.effective_until
            ? planPricing.effective_until.slice(0, 16)
            : "",
        notes: planPricing.notes || "",
    });
    ...
    const submit = (e) => {
        e.preventDefault();
        patch(
            route("admin.service.plans.pricing.update", [
                servicePlan.id,
                planPricing.id,
            ])
        );
    ...
    return (
        <AdminLayout>
            <Head title={`価格設定編集 - ${planPricing.name}`} />
