...
export default function Index({ faqs, categories, filters }) {
    ...
    const handleBulkDelete = () => {
        if (confirm("選択されたFAQを削除してもよろしいですか？")) {
            router.delete(route("admin.faq.bulk-destroy"), {
    ...
    const headerActions = [
        {
            label: PageConfig.faqs.actions.create,
            icon: PlusIcon,
            variant: "primary",
    ...
    return (
        <AdminAuthenticatedLayout
            header={
                <PageHeader
                    title={PageConfig.faqs.title}
                    description={PageConfig.faqs.description}
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {faqs.data.map((faq) => (
                            <tr key={faq.id} className="hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4">
                                    {getCategoryBadge(faq.faq_category)}
                                </td>
                                <td className="px-6 py-4">
                                    {getStatusBadge(faq)}
                                </td>
