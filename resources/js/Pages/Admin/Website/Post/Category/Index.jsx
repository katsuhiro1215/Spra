...
export default function Index({ categories, filters, flash }) {
    ...
    const handleFilter = (key, value) => {
        setData(key, value);
        get(route("admin.website.blog.category.index"), {
            preserveState: true,
            preserveScroll: true,
        });
    };
    ...
    const handleBulkAction = (action) => {
        ...
        if (confirm(confirmMessage)) {
            router.post(
                route("admin.website.blog.category.bulk-action"),
                {
                    action,
    ...
    const handleDelete = (category) => {
        if (
            ...
            router.delete(
                route("admin.website.blog.category.destroy", category.id),
            );
        }
    };
    ...
    const handleUpdateOrder = () => {
        ...
        router.post(route("admin.website.blog.category.update-order"), {
    ...
    const headerActions = [
        {
            label: PageConfig.blogCategories.actions.create,
            icon: PlusIcon,
            variant: "primary",
            route: route("admin.website.blog.category.create"),
        },
    ];
    ...
    return (
        <AdminAuthenticatedLayout>
            ...
            {/* ヘッダー */}
            <PageHeader
                title={PageConfig.blogCategories.title}
                description={PageConfig.blogCategories.description}
                                            ...
                                            <div className="flex space-x-2">
                                                <button
                                                    onClick={() =>
                                                        router.get(
                                                            route(
                                                                "admin.website.blog.category.show",
                                                                category.id,
                                                            ),
                                                        )
                                                    }
                                                ...
                                                <button
                                                    onClick={() =>
                                                        router.get(
                                                            route(
                                                                "admin.website.blog.category.edit",
                                                                category.id,
                                                            ),
                                                        )
                                                    }
                ...
                {categories.data.length === 0 && (
                    <div className="text-center py-12">
                        ...
                        <div className="mt-6">
                            <Link
                                href={route(
                                    "admin.website.blog.category.create",
                                )}
                            >
