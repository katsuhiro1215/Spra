...
- **Spraは2026-07-31付けで本番稼働中（`https://smartsprouts.jp`、実データあり）。** マイグレーションの作り直しやスキーマの自由な変更はできない。スキーマ変更は必ず後方互換・ロールバック安全な形で行うこと（このブランチのマイグレーションが実例: enum値を縮小する前に該当データを別の値へ退避してから縮める、追加カラムはnullableにする等）。
- 破壊的変更であっても可読性・命名の一貫性（ULID主体は`HasUlid`トレイト、ステータス値はモデル定数`STATUSES`/`ROLES`等を使う、ガード名は`admins`/`users`で統一）は維持すること。
