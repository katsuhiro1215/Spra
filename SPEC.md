...
| K28 | 本番環境でメディアアップロードが保存後に表示されない（Caddyが`app-storage`ボリュームにアクセスできない） | **修正済み**（2026-08-01、実運用で発覚）。`Storage::disk('public')`（`storage/app/public`）へのアップロードは`app`/`horizon`/`scheduler`コンテナ間で共有される`app-storage`ボリュームに書き込まれるが、静的ファイルを実際に配信する`caddy`コンテナは`Dockerfile.prod`のcaddyステージで`public/`をビルド時に別イメージから焼き込むだけで、`app-storage`ボリュームも`public/storage`シンボリックリンクも一切持っていなかった。そのためアップロード自体はDB・ファイルとも正常に成功するが、ブラウザから`/storage/xxx`へアクセスすると404になり画像が表示されない（`<img>`の読み込み失敗はJSエラーとして見えないため気づきにくい）。`compose.prod.yaml`のcaddyサービスに`app-storage:/var/www/html/storage`のボリュームマウントを追加し、`Dockerfile.prod`のcaddyステージに`RUN ln -sfn ../storage/app/public public/storage`（`php artisan storage:link`と同じ相対シンボリックリンク）を追加して解消 | フェーズ1（完了、実運用で顕在化） |
