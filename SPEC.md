...
- AI社員の業務ログ・日報機能として新規テーブル`ai_staff_activity_logs`（随時ログ）・`ai_staff_daily_reports`（AI社員×日で1件の日次集計）を追加。`AiStaffActivityLogger::log()`サービスが、タスクのステータス変更（`TaskController::updateStatus()`/`update()`、担当者基準）・返信(Response)作成・見積(Quote)作成・提案書(Proposal)作成の4箇所の書き込み経路末尾から呼び出され、人間Adminの操作は記録しない。毎朝8時スケジュール実行の`ai-staff:generate-daily-reports`コマンド（`routes/console.php`）が前日分のログをAI社員ごとに整形集計し`ai_staff_daily_reports`へupsertする（AI要約は行わない案A）。閲覧用の読み取り専用Admin画面（`/admin/ai-staff-daily-reports`、`Admin/AiStaffDailyReports/Index.jsx`・`Show.jsx`）を追加。本番投入時は`migrate`実行後、`AiStaffSeeder`と同様に`php artisan admin:sync-permissions`と`php artisan db:seed --class=RolePermissionSeeder`の両方を再実行しないと、新設の`ai-staff-daily-reports.index`/`.show`権限が`admin`/`owner`/`super_admin`ロールへ付与されず、既存の画面UIがルートには存在するのに全員403になる。
...
| K29 | `Admin\Admin\AdminController::destroy()`が存在しないガード名`admin`（単数形）を参照 | **修正済み**（2026-08-03、タスク管理機能の最終コードレビューで対応）。K5（`UpdateMediaRequest`）と同一パターンの取り違えが`auth('admin')->id()`という形で残存しており、管理者削除を実行すると`Auth guard [admin] is not defined.`例外が`catch (\Exception $e)`で握りつぶされ、エラーメッセージがフラッシュされるだけで管理者の削除が常に失敗していた。`auth('admins')->id()`に修正 | フェーズ2（完了） |
| K30 | Admin側「見積書PDFダウンロード」機能がクラッシュする | **未修正**（2026-08-04、領収書PDF発行者情報修正の副次調査で発覚）。`pdfs/quote.blade.php`が`QuoteVersion`ベースへの設計移行前の古いスキーマを前提としたままで、`$quote->items`（存在しないリレーション）・`$quote->base_amount`/`$quote->client_name`等（存在しない属性）を直接参照している。`QuoteController::downloadPdf()`が`$quote->load(['items' => ...])`を呼んだ時点で`RelationNotFoundException`となり必ず失敗する。修正には`pdfs/quote.blade.php`を`$quote->currentVersion->items`ベースに全面的に書き直す必要があり、今回の作業スコープ外のため未対応 | フェーズ1（要対応） |
| K31 | 見積/契約明細を「手動で追加」（ServiceItem未選択）すると保存できない | **修正済み**（2026-08-20、実運用で発覚）。`quote_items`/`contract_items`テーブルはプラン割引行等のためservice_idをnullable設計にしているのに、`QuoteItemRequest`/`ContractItemRequest`の`items.*.service_id`が`required`のままで、Admin画面の「手動で追加」でサービス未選択のまま保存すると必ずバリデーションエラーになっていた。両Requestとも`nullable`に修正、回帰テスト追加 | フェーズ1（完了） |
| K32 | 週次繰り返しタスクが、生成対象日が当日と同じ曜日の場合に重複生成される | **未修正**（2026-08-20、K31調査中に全体テストで偶発的に発覚。今日が繰り返し対象曜日と一致する日にしか再現しないため見過ごされていた）。`TaskService::generateOccurrencesForTemplate()`（週次）が、テンプレート自身の`due_date`（today）を除外せずに実体タスクを生成するケースがあり、`tests/Unit/Services/TaskRecurrenceGenerationTest.php`の`test_generates_weekly_occurrences_only_on_matching_weekdays`が今日の曜日次第で失敗する。今回の作業スコープ外のため未対応 | フェーズ2（要対応） |
