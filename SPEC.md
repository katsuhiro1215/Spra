...
| K28 | 本番環境でメディアアップロードが失敗・表示されない（原因が2つ重なっていた） | **修正済み**（2026-08-01、実運用で発覚）。実際には2つの問題が重なっていた。**(1) 真因**: 本番`app`イメージのGD拡張がWebP非対応でビルドされており（`Dockerfile.prod`の`docker-php-ext-configure gd --with-freetype --with-jpeg`に`--with-webp`が無く`libwebp-dev`も未インストール）、Intervention ImageがWebPバリアントを生成しようとする際に`Call to undefined function Intervention\Image\Drivers\Gd\Encoders\imagewebp()`で例外となりアップロード自体が失敗していた（開発環境のSailイメージはWebP対応済みのため気づかなかった）。`libwebp`/`libwebp-dev`を追加し`--with-webp`を付与して解消（`gd_info()`で`WebP Support => 1`を確認済み）。**(2) 副次的に発見**: 仮にアップロードが成功していても、静的ファイルを実際に配信する`caddy`コンテナは`app`/`horizon`/`scheduler`間で共有される`app-storage`ボリュームも`public/storage`シンボリックリンクも持っておらず、`/storage/xxx`へのアクセスが404になる状態だった（`compose.prod.yaml`のcaddyサービスへの`app-storage`ボリュームマウント追加、`Dockerfile.prod`のcaddyステージへの`RUN ln -sfn ../storage/app/public public/storage`追加で解消）。両方とも例外・404はログに出るがブラウザ上は無言で失敗するため気づきにくかった | フェーズ1（完了、実運用で顕在化） |
