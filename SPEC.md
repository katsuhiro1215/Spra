...
| K15 | Search Console連携が本番未検証 | **一部解消**（2026-07-31）。`SEARCH_CONSOLE_DRIVER=google`は従来コード上未実装で`RuntimeException`を投げるダミーだったことが判明（docsの「未検証」という記述は誤りで、実際には実装自体が存在しなかった）。`GoogleSearchConsoleService`（サービスアカウントJWT認証＋Search Analytics API直叩き、`google/apiclient`等の追加依存なし）を新規実装し、`AppServiceProvider`のバインディングを差し替え。単体テスト（`tests/Unit/Services/Analytics/GoogleSearchConsoleServiceTest.php`）追加済み。**残作業**: 本番でのサービスアカウント作成・Search Console側への権限付与・`.env`（`SEARCH_CONSOLE_DRIVER=google`/`SEARCH_CONSOLE_SITE_URL`/`SEARCH_CONSOLE_CREDENTIALS_PATH`）設定・実データ取得確認（T27） | フェーズ1（実装完了、本番接続確認は継続） |
